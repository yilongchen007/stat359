...
def sentence_to_sequence(sentence: str, vectors: KeyedVectors, max_len: int) -> np.ndarray:
    """Convert one sentence into a fixed (max_len, emb_dim) sequence."""
    ...
    for idx, token in enumerate(tokens[:max_len]):
        try:
            seq[idx] = vectors.get_vector(token)
        except KeyError:
            continue
...
def build_sequences(sentences: list[str], vectors: KeyedVectors, max_len: int) -> np.ndarray:
    """Build tensor of shape (n_samples, max_len, emb_dim)."""
    sequences = [sentence_to_sequence(sent, vectors, max_len) for sent in sentences]
...
class SequenceDataset(Dataset):
    """Torch dataset for sequence embedding inputs."""
...
class LSTMClassifier(nn.Module):
    """LSTM classifier over precomputed (32, 300) token vectors."""
...
def plot_curves(history: dict[str, list[float]], output_dir: str) -> None:
    ...
    plt.xlabel('Epoch')
    plt.ylabel('Macro F1')
    plt.title('LSTM Macro F1 Curve')
    plt.grid(True)
    plt.legend()

    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, 'lstm_learning_curves.png'), dpi=150)
    plt.close()
...
def train(args: argparse.Namespace) -> None:
    ...
    print('Building padded/truncated FastText sequences...')
    sequences = build_sequences(sentences.tolist(), vectors, MAX_LEN)

    x_train, y_train, x_val, y_val, x_test, y_test = stratified_splits(sequences, labels, args.seed)
    ...
    best_epoch = -1
    stale_epochs = 0
    best_model_path = os.path.join(args.output_dir, 'best_lstm_model.pth')
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Train LSTM sentiment classifier (FastText padded sequences).')
